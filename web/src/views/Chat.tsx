...
function AttCard({ a, url }: { a: Att; url: string }) {
    ...
    return (
        <a className="msg-att" href={url} target="_blank" rel="noreferrer">
            ...
            <span className="grow">
                {a.filename}
                {vErr ? i18n.t("chat.videoUnsupported") : ""}
            </span>
            <span className="asz">{fmtSize(a.sizeBytes)}</span>
        </a>
    );
        ...
        <div className="msg-rx">
            {rs.map((r) => {
                ...
                return (
                    <button
                        key={r.emoji}
                        className={"rx-chip" + (did ? " on" : "")}
                        onClick={() => onReact(r.emoji, !!did)}
                    >
                        {r.emoji} {r.count}
                        {names ? (
                            <span className="rx-tip" role="tooltip">
                                {names}
                            </span>
                        ) : null}
                    </button>
...
function ActionCardMsg({ m }: { m: Msg }) {
    ...
    const { createChannel, markActionExecuted, slug, agents, attachmentUrl } =
        useStore();
    ...
    return (
        <div className="msg action-card-msg" id={"m-" + m.id} key={m.id}>
            <Avatar
                seed={m.senderName}
                url={resolveAvatar(
                    agents.find((a) => a.id === m.senderId)?.avatarUrl,
                    attachmentUrl,
                )}
                size={36}
            />
            <div className="msg-col">
                <div className="msg-head">
                    ...
                    <span className="member-badge">{t("chat.proposed")}</span>
            ...
            {open && isChan && (
                <CreateChannelModal
                    prefill={{
                        ...
                        agentIds: a.initialAgents ?? [],
                        userIds: a.initialHumans ?? [],
                    }}
                    submitLabel={t("chat.createChannel")}
                    onClose={() => setOpen(false)}
                    onCreate={async (opts) => {
                        const r = await createChannel(opts);
                        setOpen(false);
                        if (r?.id) {
                            await markActionExecuted(m.id, {
                                kind: "channel",
                                id: r.id,
                                name: opts.name,
                            });
                            nav(`/s/${slug}/channel/${r.id}`);
                        }
                    }}
                />
            )}
            {open && !isChan && (
                <CreateAgentModal
                    prefill={{ name: a.name, description: a.description ?? "" }}
                    onClose={() => setOpen(false)}
                    onCreated={async (r) => {
                        await markActionExecuted(m.id, {
                            kind: "agent",
...
export function Chat() {
    ...
    const senderAvatar = (m: Msg) =>
        avFor(
            m.senderType === "agent"
                ? agents.find((a) => a.id === m.senderId)?.avatarUrl
                : humans.find((h) => h.userId === m.senderId)?.avatarUrl,
        );
    ...
    const [profile, setProfile] = useState<{
        type: "agent" | "human";
        id: string;
    } | null>(null); // right-column profile overlay: clicking an avatar / name / @mention (agent, human, or yourself) opens it ON TOP of the thread/trajectory ("click X → show X"); closing it reveals the layer underneath
    ...
    const [hoverAgent, setHoverAgent] = useState<{
        id: string;
        x: number;
        y: number;
    } | null>(null); // hovering over an agent shows a quick-info hover card
    const [ctxMenu, setCtxMenu] = useState<{
        m: Msg;
        x: number;
        y: number;
    } | null>(null); // right-clicking a message opens the context action menu
    ...
    const [thread, setThread] = useState<{
        channelId: string;
        parent: Msg;
    } | null>(null); // currently open thread panel
    const [threadMeta, setThreadMeta] = useState<
        ...
        >
    >({}); // parent message id → thread metadata (reply count, unread count)
    ...
    >([]); // unread that lives in this channel's threads (invisible in the main timeline) → "jump to unread thread" bar
    ...
    const cur =
        [...channels, ...dms].find((c) => c.id === channelId) ||
    ...
    const dmAgent =
        dmPeer?.peerType === "agent"
            ? agents.find((a) => a.id === dmPeer.peerId)
            : undefined; // DM peer agent → used for the live status indicator in the header
    ...
    useEffect(() => {
        if (!channelId && cur)
            nav(`/s/${slug}/channel/${cur.id}`, { replace: true });
    }, [channelId, cur, slug, nav]);
    useEffect(() => {
        if (!cur) return;
        setThread(null);
        setProfile(null);
        ...
        subscribeChannel(cur.id);
        (async () => {
            // switching channels closes any open thread + profile overlay from the previous channel (loaded/msgs/hasMore reset happens synchronously in the render-phase guard above) (the live trace itself persists — accumulated in the store, see store.tsx); join the room while viewing so message:new arrives live (covers public non-member channels + channels relevant after connect)
            const d = await api(
                "GET",
                `/api/messages/channel/${cur.id}?limit=${PAGE_SIZE}`,
            );
            ...
            if (ids.length) {
                try {
                    setThreadMeta(
                        (await api(
                            "GET",
                            `/api/channels/${cur.id}/threads?parentMessageIds=${ids.join(",")}`,
                        )) || {},
                    );
                } catch {
                    setThreadMeta({});
    ...
    useEffect(() => {
        if (!cur || cur.type === "dm" || cur.type === "thread") {
            setUnreadThreads([]);
            return;
        }
        let cancelled = false;
            ...
            try {
                ...
                setUnreadThreads(
                    (r?.threads || [])
                        .filter(
                            (th: any) =>
                                th.parentChannelId === cur.id &&
                                th.unreadCount > 0 &&
                                th.parentMessageId,
                        )
                        .map((th: any) => ({
                            threadChannelId: th.threadChannelId,
                            parentMessageId: th.parentMessageId,
                            parentChannelId: th.parentChannelId,
    ...
    useEffect(
        () =>
            onEvent((e) => {
                if (e.type === "message" && e.channelId === cur?.id) {
                    const idx = Math.min(burstCountRef.current, 7);
                    newMsgOrderRef.current.set(e.message.id, idx);
                    ...
                    markRead(cur.id);
                } // don't trim mid-pagination: a trim's setHasMore(true) would race the in-flight loadOlder's setHasMore — suppressing it closes the window (the next message trims instead)
                else if (e.type === "message:updated" && e.message)
                    setMsgs((m) =>
                        m.map((x) =>
                            x.id === e.message.id ? { ...x, ...e.message } : x,
                        ),
                    ); // sync reactions and task fields
                else if (e.type === "thread:updated" && e.parentMessageId)
                    setThreadMeta((tm) => ({
                        // live reply count update; unreadCount is approximated from the replyCount delta (the authoritative value is corrected on channel switch via GET)
                        ...tm,
                        [e.parentMessageId]: nextThreadMeta(
                            tm[e.parentMessageId],
                            {
                                threadChannelId: e.threadChannelId,
                    ...
                    setSub(
                        e.activity
                            ? `${e.name} · ${e.activity}${e.detail ? " · " + e.detail : ""}`
                            : "",
                    ); // live-trace entries are accumulated globally in the store (see store.tsx agent:activity handler), so they persist across channel/DM switches
            }),
        [cur?.id],
    );
    useEffect(() => {
        const el = scrollRef.current;
        if (!el || msgParam) return;
        if (atBottomRef.current) el.scrollTop = el.scrollHeight;
    }, [msgs, msgParam]); // auto-scroll only when already pinned to the bottom
    ...
    useLayoutEffect(() => {
        const el = scrollRef.current;
        if (el && prependRestoreRef.current != null) {
            el.scrollTop = el.scrollHeight - prependRestoreRef.current;
            prependRestoreRef.current = null;
        }
    }, [msgs]);
    useEffect(() => {
    ...
    }, [msgs]); // a live-tail trim opened a gap at the top → older messages stay re-fetchable
    useEffect(() => {
        atBottomRef.current = true;
        setShowJump(false);
        newMsgOrderRef.current.clear();
        burstCountRef.current = 0;
    }, [cur?.id]); // reset bottom-pin state + new-msg enter animation tracking on channel switch
    const toBottom = () => {
        const el = scrollRef.current;
    ...
    const loadOlder = async () => {
        ...
        try {
            const d = await api(
                "GET",
                `/api/messages/channel/${chId}?limit=${PAGE_SIZE}&before=${msgs[0]!.seq}`,
            );
    ...
    useEffect(() => {
        // scroll to and highlight the target message for ~2s when msgParam is set (once per target)
        ...
        if (el) {
            highlightedMsgRef.current = msgParam;
            el.scrollIntoView({ block: "center" });
            el.classList.add("msg-hl");
            setTimeout(() => el.classList.remove("msg-hl"), 2200);
        } // no cleanup-cancel: the removal must outlive re-renders, else a re-render cancels the timer and the highlight sticks
    ...
    useEffect(() => {
        // ?thread= auto-opens the thread panel: finds the parent message (full id or 8-char short id) in the loaded list and calls startThread; each threadParam is only opened once
        if (!threadParam || !msgs.length) return;
        if (thread) return; // panel already open, do not re-open
        const short = threadParam.includes(":")
            ? threadParam.split(":").pop()!
            : threadParam;
        const m = msgs.find(
            (x) => x.id === threadParam || x.id.startsWith(short),
        );
        if (m) startThread(m);
    ...
    const setTab = (t: string) => {
        ...
        if (t === "chat") n.delete("chatTab");
        else n.set("chatTab", t);
        setSp(n, { replace: true });
    };
    const doDM = async (agentId: string) => {
        const id = await openDM("agent", agentId);
        if (id) nav(`/s/${slug}/channel/${id}`);
    }; // used by AgentProfile onMessage callback
    const doDMHuman = async (uid: string) => {
        const id = await openDM("user", uid);
        if (id) nav(`/s/${slug}/channel/${id}`);
    }; // used by HumanProfile onMessage callback
    ...
    const startThread = async (m: Msg) => {
        if (!cur) return;
        const tid =
            threadMeta[m.id]?.threadChannelId ||
            (await openThread(cur.id, m.id));
        if (tid) {
            setProfile(null);
            setThread({ channelId: tid, parent: m });
            setThreadMeta((tm) =>
                tm[m.id]
                    ? { ...tm, [m.id]: { ...tm[m.id]!, unreadCount: 0 } }
                    : tm,
            );
            markRead(tid);
        }
    }; // opening a thread clears the unread count optimistically and marks the thread channel as read
    ...
    const openUnreadThread = async (item: {
        threadChannelId: string;
        parentMessageId: string;
    }) => {
        if (!cur) return;
        try {
            const parent = (
                await api("GET", `/api/messages/${item.parentMessageId}`)
            )?.message as Msg | undefined;
            if (!parent) return;
            setProfile(null);
            ...
            setThreadMeta((tm) =>
                tm[parent.id]
                    ? {
                          ...tm,
                          [parent.id]: { ...tm[parent.id]!, unreadCount: 0 },
                      }
                    : tm,
            );
            markRead(item.threadChannelId);
            setUnreadThreads((list) =>
                list.filter(
                    (th) => th.threadChannelId !== item.threadChannelId,
                ),
            );
        } catch {
            /* parent fetch failed (deleted / no access) — leave the bar untouched */
        }
    };
    // Returns the display name of the task assignee, used by the task pill
    const taskAssignee = (m: Msg) => {
        if (!m.taskAssigneeId) return "";
        const a = agents.find((x) => x.id === m.taskAssigneeId);
        if (a) return " @" + (a.displayName || a.name);
        const h = humans.find((x) => x.userId === m.taskAssigneeId);
        return h ? " @" + (h.displayName || h.name) : "";
    };
    // Handles task status change / claim from the task badge; socket message:updated event refreshes the message automatically
    const doTask = async (m: Msg, action: string, body?: unknown) => {
        try {
            await api("PATCH", `/api/tasks/${m.id}/${action}`, body);
        } catch {
            /* will self-correct on next reload */
        }
    };
    ...
    const navToken = async (type: string, args: string[]) => {
        if (type === "agent")
            return setProfile({ type: "agent", id: args[0]! });
        if (type === "human")
            return setProfile({ type: "human", id: args[0]! }); // @human click → profile panel (same overlay as agents, not a full-page route)
        if (type === "channel") return nav(`/s/${slug}/channel/${args[0]}`);
        if (type === "thread")
            return nav(
                `/s/${slug}/channel/${args[0]}?thread=${args[0]}:${args[1]}`,
            );
        if (type === "task") {
            const num = Number(args[0]);
            const local = msgs.find((x) => x.taskNumber === num);
            if (local && cur)
                return nav(`/s/${slug}/channel/${cur.id}?msg=${local.id}`);
            try {
                const r = await api("GET", "/api/tasks/server");
                const tk = (r?.tasks ?? r ?? []).find(
                    (x: any) => x.taskNumber === num,
                );
                if (tk) nav(`/s/${slug}/channel/${tk.channelId}?msg=${tk.id}`);
            } catch {
                /* */
            }
        }
    ...
    return (
        <>
            ...
            <main className="content-col">
                <div className="head chat-head">
                    ...
                    {dmAgent ? (
                        <span className="head-status">
                            <span
                                className={
                                    "dot " + (dmAgent.activity || "offline")
                                }
                            />
                            {dmAgent.activityDetail ||
                                dmAgent.activity ||
                                "offline"}
                        </span>
                    ) : (
                ...
                {chatTab === "tasks" && cur ? (
                    <TaskBoard channelId={cur.id} onOpenThread={startThread} />
                ) : chatTab === "files" && cur ? (
                    <ChannelFiles channelId={cur.id} />
                ) : (
                    <>
                        {chatTab === "chat" && unreadThreads.length > 0 && (
                            <button
                                className="unread-threads-bar"
                                onClick={() =>
                                    openUnreadThread(unreadThreads[0]!)
                                }
                            >
                                <MessageCircle size={14} />
                                <span className="utb-label">
                                    {t("chat.unreadThreads", {
                                        count: unreadThreads.reduce(
                                            (s, th) => s + th.unreadCount,
                                            0,
                                        ),
                                    })}
                                </span>
                                <span className="utb-cta">
                                    {t("chat.viewUnreadThread")}
                                </span>
                            </button>
                        )}
                        <div
                            key={cur?.id}
                            className="scroll ch-view-enter"
                            ref={scrollRef}
                            onScroll={onScroll}
                        >
                            {!loaded && <ChatSkeleton />}
                            ...
                            {msgs.map((m) => {
                                const ag =
                                    m.senderType === "agent" && m.senderId
                                        ...
                                        : undefined; // used for role description and avatar status dot
                                const tm = threadMeta[m.id];
                                const isMember =
                                    m.senderType !== "agent" &&
                                    m.senderType !== "system"; // human/user senders get a "member" badge
                                // action card (agent proposal card) → rendered by dedicated ActionCardMsg component
                                if (
                                    m.messageType === "action" &&
                                    m.actionMetadata?.kind === "action-card"
                                )
                                    return <ActionCardMsg m={m} key={m.id} />;
                                ...
                                const staggerIdx = newMsgOrderRef.current.get(
                                    m.id,
                                );
                                            ...
                                            {!!m.content && (
                                                <div className="mbody">
                                                    <MessageContent
                                                        ...
                                                        channels={channels}
                                                        nav={navToken}
                                                        senderType={m.senderType === "agent" || m.senderType === "human" ? m.senderType : undefined}
                                                    />
                                                </div>
                                            )}
                        ...
                        {showJump && (
                            <button className="jump-bottom" onClick={toBottom}>
                                <ArrowDown size={14} /> {t("chat.backToBottom")}
                            </button>
                        )}
                        {cur?.type === "showcase" ? (
                            <div className="showcase-readonly">
                                <Eye size={14} />
                                {t("chat.showcaseReadOnly")}
                            </div>
                        ) : (
                            <Composer
                                channelId={cur?.id ?? ""}
                                placeholder={
                                    isDm
                                        ? t("chat.dmPlaceholder", {
                                              name: cur?.name,
                                          })
                                        : t("chat.channelPlaceholder")
                                }
                                allowAsTask
            ...
            {profile ? (
                ...
                <aside className="traj-col">
                    ...
                    {traj.length === 0 ? (
                        ...
                        traj.map((t, i) => (
                            <div
                                className={"traj" + (t.tool ? " tool" : "")}
                                key={i}
                            >
                                {t.tool && <IconWrench size={12} />}
                                {t.name ? "@" + t.name + " · " : ""}
                                {t.text}
                            </div>
                        ))
            ...
            {hoverAgent &&
                (() => {
                    ...
                    if (!a) return null;
                    const live =
                        (a.activity && a.activity !== "offline"
                            ? a.activity
                            : a.status) || "offline";
                    return (
                        <div
                            className="agent-hovercard"
                            style={{
                                left: Math.min(
                                    hoverAgent.x,
...
function EditChannelModal({
    ...
    const save = async () => {
        ...
        setSaving(true);
        try {
            await api("PATCH", `/api/channels/${channel.id}`, {
                name: name.trim(),
    ...
    const doDelete = async () => {
        if (
            !(await confirm({
                title: t("chat.deleteChannelTitle", { name: channel.name }),
        ...
        <div className="modal-bg" onClick={onClose}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
                ...
                >
                    ...
                    <button
                        className="ch-act ch-act-danger"
                        onClick={doArchive}
                    >
...
function ThreadPanel({
    ...
    const navToken = async (type: string, args: string[]) => {
        ...
        if (type === "task") {
            try {
                const r = await api("GET", "/api/tasks/server");
                const tk = (r?.tasks ?? r ?? []).find(
                    (x: any) => x.taskNumber === Number(args[0]),
                );
                if (tk) nav(`/s/${slug}/channel/${tk.channelId}?msg=${tk.id}`);
            } catch {
                /* */
    ...
    useEffect(() => {
        ...
        (async () => {
            const d = await api(
                "GET",
                `/api/messages/channel/${channelId}?limit=200`,
            );
            setMsgs(d.messages || []);
        })();
    ...
    useEffect(
        () =>
            onEvent((e) => {
                if (e.type === "message" && e.channelId === channelId)
                    setMsgs((m) => [...m, e.message]);
                else if (
                    e.type === "message:updated" &&
                    e.message?.channelId === channelId
                )
                    setMsgs((m) =>
                        m.map((x) =>
                            x.id === e.message.id ? { ...x, ...e.message } : x,
    ...
    const row = (m: Msg) => {
        if (m.senderType === "system")
            ...
            ); // system messages render as a banner with no avatar
        const ag =
            m.senderType === "agent" && m.senderId
                ? agents.find((a) => a.id === m.senderId)
                : undefined; // agent sender → avatar and name are clickable to open the profile panel
        return (
            <div className="msg" key={m.id}>
                {ag ? (
                    <span
                        className="msg-av clickable"
                        onClick={() => onOpenProfile("agent", m.senderId!)}
                    >
                        <Avatar
                            seed={m.senderName}
                            url={senderAvatar(m)}
                    ...
                    <span
                        className="msg-av clickable"
                        onClick={() => onOpenProfile("human", m.senderId!)}
                    >
                        <Avatar
                            seed={m.senderName}
                            url={senderAvatar(m)}
                ...
                <div className="msg-col">
                    <div>
                        {ag ? (
                            <span
                                className="who clickable"
                                onClick={() =>
                                    onOpenProfile("agent", m.senderId!)
                                }
                            >
                                {m.senderName}
                            </span>
                            ...
                            <span
                                className="who clickable"
                                onClick={() =>
                                    onOpenProfile("human", m.senderId!)
                                }
                            >
                                {m.senderName}
                            </span>
                    ...
                    <Reactions
                        m={m}
                        mine={me?.id ?? ""}
                        onReact={(emoji, remove) => react(m.id, emoji, remove)}
                    />
                </div>
            </div>
        );
    ...
    return (
        <aside className="thread-panel">
            <div className="thread-head">
                ...
                <button
                    className="tp-link"
                    title={t("chat.markDone")}
                    onClick={async () => {
                        await api("POST", "/api/channels/threads/done", {
                ...
                <button
                    className="tp-link"
                    title={t("chat.unfollowThread")}
                    onClick={async () => {
                        await api("POST", "/api/channels/threads/unfollow", {
                            threadChannelId: channelId,
                        });
                        onClose();
                    }}
                ...
                <button
                    ...
                    onClick={() =>
                        nav(
                            `/s/${slug}/channel/${parent.channelId}?msg=${parent.id}`,
                        )
                    }
                    title={t("chat.viewInChannel")}
                >
            ...
            <div className="scroll" ref={scrollRef}>
                ...
                <div className="thread-sep">
                    {t("chat.replyCount", { count: msgs.length })}
                </div>
                {msgs.map(row)}
            </div>
            {channels.find((c) => c.id === parent.channelId)?.type ===
            "showcase" ? (
                <div className="showcase-readonly">
                    <Eye size={14} />
                    {t("chat.showcaseReadOnly")}
...
function ChannelMembersModal({
    ...
    const {
        ...
        attachmentUrl,
        capabilities,
    } = useStore(); // visibleAgents: showcase demo props are not offered in the "add agent" list
    const avFor = (u?: string | null) => resolveAvatar(u, attachmentUrl);
    const [data, setData] = useState<{ agents: any[]; humans: any[] }>({
        agents: [],
        humans: [],
    });
    const load = async () => {
        const d = await api("GET", `/api/channels/${channelId}/members`);
        setData({ agents: d?.agents || [], humans: d?.humans || [] });
    };
    useEffect(() => {
        load();
    }, [channelId]);
    ...
    const add = async (agentId: string) => {
        await api("POST", `/api/channels/${channelId}/members`, { agentId });
        load();
    };
    const remove = async (agentId: string) => {
        await api("DELETE", `/api/channels/${channelId}/members`, { agentId });
        load();
    };
    return (
        <div className="modal-bg" onClick={onClose}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
                <h3>
                    # {channelName} ·{" "}
                    {t("chat.membersCount", {
                        count: data.agents.length + data.humans.length,
                    })}
                </h3>
                <div className="sec">
                    {t("common.agents")}{" "}
                    <span className="cnt">{data.agents.length}</span>
                </div>
                {data.agents.map((a) => (
                    <div key={a.id} className="item">
                        <Avatar
                            ...
                            size={22}
                        />
                        <span className="grow">{a.displayName || a.name}</span>
                        <span className={"dot " + (a.activity || a.status)} />
                        {capabilities.manageChannels && (
                            <button
                                className="joinbtn"
                                onClick={() => remove(a.id)}
                ...
                <div className="sec">
                    {t("common.humans")}{" "}
                    <span className="cnt">{data.humans.length}</span>
                </div>
                {data.humans.map((u) => (
                    <div key={u.userId} className="item">
                        <Avatar
                            seed={u.name}
                            url={avFor(u.avatarUrl)}
                            size={22}
                        />
                        <span className="grow">{u.displayName || u.name}</span>
                    </div>
                ))}
                {capabilities.manageChannels && addable.length > 0 && (
                    <>
                        ...
                        {addable.map((a) => (
                            <div key={a.id} className="item ghost">
                                <Avatar
                                    seed={a.name}
                                    url={avFor(a.avatarUrl)}
                                    size={22}
...
function ChannelFiles({ channelId }: { channelId: string }) {
    ...
    useEffect(() => {
        (async () => {
            const d = await api("GET", `/api/channels/${channelId}/files`);
            setFiles(d?.files || []);
        })();
    }, [channelId]);
    return (
        <div className="scroll ch-view-enter">
            {files.length === 0 ? (
                <div className="empty">{t("chat.noFiles")}</div>
            ) : (
                files.map((f) => (
                    <div key={f.id} className="card file-row">
                        ...
                        >
                            ...
                            <div className="grow">
                                <div className="who">{f.filename}</div>
                                <div className="meta">
                                    {fmtSize(f.sizeBytes)} ·{" "}
                                    {f.uploader?.displayName ||
                                        f.uploader?.name ||
                        ...
                        <div className="file-acts">
                            {f.messageId && (
                                <button
                                    ...
                                    onClick={() =>
                                        nav(
                                            `/s/${slug}/channel/${f.channelId}?msg=${f.messageId}`,
                                        )
                                    }
                                >
